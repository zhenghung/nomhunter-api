...
export class MissionTransformer {
  static map(
    missionGroupEntities: MissionGroupEntity[]
  ): MissionGroupsInterface {
    const missionGroupsMapped: MissionGroupInterface[] = missionGroupEntities.map(
      (missionGroupEntity) => {
        const missionsMapped: MissionInterface[] = missionGroupEntity.missions.map(
          (missionEntity) => {
            ...
            if (missionEntity.rewardExp > 0) {
              rewardsMapped.push({
                type: "reward.exp",
                rewardImageUrl:
                  "https://static.odealo.com/uploads/auction_images//13497874885e839ed4a3b4f.png",
              });
            }
            ...
            const missionMapped: MissionInterface = {
              ...
              currentProgress:
                missionEntity.playerMissions.length > 0
                  ? missionEntity.playerMissions[0].currentProgress
                  : 0,
              maxProgress: missionEntity.maxProgress,
              completed:
                missionEntity.playerMissions.length > 0
                  ? missionEntity.playerMissions[0].completed
                  : false,
              claimed:
                missionEntity.playerMissions.length > 0
