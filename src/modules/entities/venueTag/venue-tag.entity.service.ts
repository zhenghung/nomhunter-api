...
@Injectable()
export class VenueTagEntityService extends GenericEntityService<VenueTagEntity> {
  constructor(
    @InjectRepository(VenueTagEntity)
    private readonly venueTagEntityRepository: Repository<VenueTagEntity>
  ) {
    super(venueTagEntityRepository, new Logger(VenueTagEntityService.name), VenueTagEntity.name);
  }
   ...
   */
  async createMany(createManyVenueTagsDto: CreateManyVenueTagsDto): Promise<VenueTagEntity[]> {
    const venueTags: VenueTagEntity[] = createManyVenueTagsDto.tags.map((tag) => {
